...
import 'baidu_translation_api.dart';
...
void main() async {
  ...
  for (var file in files) {
    if (file is File && file.path.endsWith('.dart')) {
      final content = await file.readAsString();_extractTranslations(content, translations);
    }
  }
...
Future<String?> _translation(String value, String to) async {
  ...
  try{
    ...
    var sign = "${params['appid']}${params['q']}${params['salt']}${BaiduTranslationApi.key}";

    var bytes = utf8.encode(sign);
    var digest = md5.convert(bytes);
    params["sign"] = digest.toString();

    var resp = await dio.get('https://fanyi-api.baidu.com/api/trans/vip/translate',queryParameters: params);
