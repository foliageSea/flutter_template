...
class ResultMapper extends ClassMapperBase<Result> {
  ...
  @override
  Function get typeFactory => <T>(f) => f<Result<T>>();
  ...
  static const Field<Result, dynamic> _f$data =
  ...
  static Result<T> _instantiate<T>(DecodingData data) {
    return Result(data.dec(_f$code), data.dec(_f$success), data.dec(_f$message),
        data.dec(_f$data));
  }
...
mixin ResultMappable<T> {
  String toJson() {
    return ResultMapper.ensureInitialized()
        .encodeJson<Result<T>>(this as Result<T>);
  }

  Map<String, dynamic> toMap() {
    return ResultMapper.ensureInitialized()
        .encodeMap<Result<T>>(this as Result<T>);
  }

  ResultCopyWith<Result<T>, Result<T>, Result<T>, T> get copyWith =>
      _ResultCopyWithImpl<Result<T>, Result<T>, T>(
          this as Result<T>, $identity, $identity);
  @override
  String toString() {
  ...
  bool operator ==(Object other) {
    return ResultMapper.ensureInitialized()
        .equalsValue(this as Result<T>, other);
  }
